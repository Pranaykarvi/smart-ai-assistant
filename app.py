...
def display_code_output(code: str):
    ...
    match = re.search(r"```(?:\w+)?\n([\s\S]+?)```", code)
    code_content = match.group(1).strip() if match else code.strip()
...
if agent == "General Q&A":
    ...
    if st.button("Translate"):
        ...
        st.text_area("Translated:", response, height=300)
        text_to_speech(response)
    ...
    if st.button("Summarize"):
        ...
        st.text_area("Summary:", response, height=300)
        text_to_speech(response)
    ...
    if st.button("Show Reminders"):
        ...
        st.text_area("Reminders:", reminder_list, height=300)
    ...
    if st.button("List Files"):
        ...
        st.text_area("Files:", file_list, height=300)
    ...
    if st.button("Explain / Get Stock Price"):
        if term.strip() == "":
            st.warning("Please enter a valid finance term or stock symbol.")
        else:
            if term.isalpha() and term.isupper() and len(term) <= 5:
                # Looks like stock symbol
                response = finance_agent.get_stock_price(term)
            else:
    ...
    if st.button("🎧 Listen Now"):
        ...
        if raw_command:
            ...
            st.text_area("Automation Response:", response, height=300)
            text_to_speech(response)
        else:
